    ...
    return (
        <div className="flex flex-col gap-2 w-11/12 md:w-11/12 lg:w-3/4 xl:w-7/12 px-2 sm:px-10 py-5 sm:py-10 rounded-lg glassmorphism mt-24 mb-32 sm:my-0 ">
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-2 px-1'>
                ...
                <textarea name="" id="" className='min-h-44 rounded-md p-1 px-2 outline-none bg-gray-50' placeholder='Enter Your Prompt here ... ' onChange={(e) => { setPost({ ...{ post }, prompt: e.target.value }) }}></textarea>
                <label htmlFor="">Tags</label>
                <input type="text" className='h-10 rounded-md p-1 px-2 outline-none bg-gray-50' placeholder='Enter Your Tags here ... ' onChange={(e) => { setPost({ ...{ post }, tag: e.target.value }) }} />
